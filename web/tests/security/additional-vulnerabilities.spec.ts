...
test.describe.skip(
  '🚨 Additional Critical Security Vulnerabilities Tests',
  () => {
    ...
    test.describe('OAuth Information Disclosure Prevention', () => {
      test('should not expose OAuth configuration details', async ({
        page,
      }) => {
    ...
    test.describe('Enhanced Callback URL Validation', () => {
      ...
      test('should prevent path traversal via callback URL', async ({
        page,
      }) => {
        const traversalUrls = [
    ...
    test.describe('Secure-by-Default Middleware', () => {
      ...
      test('public API routes should remain accessible', async ({
        request,
      }) => {
