...
export function SearchAndFilterBar() {
  ...
  const hasActiveFilters = search || status !== 'all' || sortBy !== 'createdAt' || sortOrder !== 'desc' || selectedTags.length > 0
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <Select value={`${sortBy}-${sortOrder}`} onValueChange={(value) => {
            ...
            <SelectContent>
              ...
              <SelectItem value="responseCount-desc">回答数（多い順）</SelectItem>
              <SelectItem value="responseCount-asc">回答数（少ない順）</SelectItem>
            </SelectContent>
          </Select>
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap gap-2">
          ...
          {status !== 'all' && (
            <Badge variant="secondary" className="gap-1">
              ステータス: {status === 'DRAFT' ? '下書き' : status === 'PUBLISHED' ? '公開済み' : 'アーカイブ'}
              <X 
                className="w-3 h-3 cursor-pointer" 
