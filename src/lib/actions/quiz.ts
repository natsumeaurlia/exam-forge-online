...
import { QuizStatus, ScoringType, SharingMode, QuestionType } from '@prisma/client'
...
const updateQuizSchema = z.object({
  ...
  subdomain: z.string().min(3, 'サブドメインは3文字以上必要です').max(30, 'サブドメインは30文字以下にしてください').regex(/^[a-z0-9-]+$/, '小文字、数字、ハイフンのみ使用可能です').optional(),
...
const publishQuizSchema = z.object({
  id: z.string(),
  subdomain: z.string().min(3, 'サブドメインは3文字以上必要です').max(30, 'サブドメインは30文字以下にしてください').regex(/^[a-z0-9-]+$/, '小文字、数字、ハイフンのみ使用可能です'),
})
...
const getQuizzesSchema = z.object({
  ...
  sortBy: z.enum(['title', 'createdAt', 'updatedAt', 'responseCount']).default('createdAt'),
...
const addQuestionSchema = z.object({
  quizId: z.string(),
  type: z.enum(['TRUE_FALSE', 'MULTIPLE_CHOICE', 'CHECKBOX', 'SHORT_ANSWER', 'SORTING', 'FILL_IN_BLANK', 'DIAGRAM', 'MATCHING', 'NUMERIC']),
...
export const updateQuiz = action
  ...
  .action(async ({ parsedInput: data }) => {
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : 'クイズの更新に失敗しました')
    }
  })
...
export const deleteQuiz = action
  ...
  .action(async ({ parsedInput: data }) => {
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : 'クイズの削除に失敗しました')
    }
  })
...
export const publishQuiz = action
  ...
  .action(async ({ parsedInput: data }) => {
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : 'クイズの公開に失敗しました')
    }
  })
...
export const addQuestion = action
  ...
  .action(async ({ parsedInput: data }) => {
    ...
    try {
      ...
      // 次の順序番号を計算
      const maxOrder = quiz.questions.reduce((max: number, q: any) => Math.max(max, q.order), 0)
      ...
      throw new Error(error instanceof Error ? error.message : '問題の追加に失敗しました')
    }
  })
...
export const updateQuestion = action
  ...
  .action(async ({ parsedInput: data }) => {
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : '問題の更新に失敗しました')
    }
  })
...
export const deleteQuestion = action
  ...
  .action(async ({ parsedInput: data }) => {
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : '問題の削除に失敗しました')
    }
  })
...
export const reorderQuestions = action
  ...
  .action(async ({ parsedInput: data }) => {
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : '問題順序の変更に失敗しました')
    }
  })
